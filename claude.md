...
* **Demo reset key (main.js keydown):** **"4" resets every window to its original position/scale/morph/visibility** so the demo can be re-run without a page reload. Windows ONLY — the menubar and grid are untouched. Also cancels in-flight window animations/drag and restores stack order (via `windowsApi.resetStack()` from `initWindows`) so the next click doesn't reshuffle z.
