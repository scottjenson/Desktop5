...
2. ~~**Morphed dragging "runs away" from the cursor** and stops short in the flank.~~ **FIXED (2026-07-02, windows.js)** as part of the morph-readability work (`plans/morph-readability.md`): the physical cursor is forward-warped into logical space (`cursorToLogical`, essentially the `screenToLogicalCoords` machinery below), `mesh.scale` is frozen during a morphed drag (the warp does all the compressing — applying `getWindowScale` on top double-shrank), and the drag clamp uses the logical-overscroll bounds documented below. Remaining gap: raycaster HIT-TESTING still targets the un-morphed quad, so re-grabbing a morphed window parked deep in a flank means clicking its logical (not visual) position.
