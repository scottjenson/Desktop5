...
* **Environment:** highly experimental API. The user runs **standard-release Chrome (150+, stable channel — NOT Canary)** with `chrome://flags/#canvas-draw-element` enabled; the API is in an Origin Trial (Chrome 148+) and still changing shape. No other-browser fallbacks needed; assume `layoutsubtree` and `requestPaint` work.
...
**Next — in-window interactivity (Phase B):** map raycaster `uv` → window-local px → `document.elementFromPoint` → dispatch synthetic click/hover so buttons, tabs, and hover states work. This is also the enabler for file-level dragging. **De-risked by Learning #13** (wp text selection): the reparented DOM IS hit-testable in gBCR space — reuse `caretFromLocal()`'s occlusion-lift + viewport-scroll pattern, and remember synthetic events can't trigger native selection/focus behaviors.
